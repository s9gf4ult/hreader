...
instance (MonadHReader m) => MonadHReader (ReaderT (HSet els) m) where
  type MHRElements (ReaderT (HSet els) m) = MHRElements m
  askHSet = lift askHSet
  -- hlocal :: (HSet els -> HSet els) -> m a -> m a
  hlocal f ma = ReaderT.local f ma
